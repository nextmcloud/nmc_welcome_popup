...
namespace OCA\NMC_Welcome_Popup\Notification;
...
class AppHint  {
	...
	public function sendAppHintNotifications(): void {
		if ($this->userId !== null && $this->groupManager->isAdmin($this->userId) && $this->config->getUserValue($this->userId, 'nmc_welcome_popup', 'apphint') !== self::APP_HINT_VERSION) {
			...
			$this->config->setUserValue($this->userId, 'nmc_welcome_popup', 'apphint', self::APP_HINT_VERSION);
		}
	}
	...
	protected function sendNotification(string $app, string $user): void {
		...
		if (
			$this->config->getUserValue($this->userId, 'nmc_welcome_popup', 'apphint') !== self::APP_HINT_VERSION
	...
	public function dismissNotification(string $app) {
		...
		$notification->setApp('nmc_welcome_popup')
