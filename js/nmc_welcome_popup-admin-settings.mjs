...
var de = "de_DE";
(function(OCA2) {
  OCA2.NMC_Welcome_Popup = OCA2.NMC_Welcome_Popup || {};
  OCA2.NMC_Welcome_Popup.Admin = {
    currentConfig: "1",
    slideIds: "1",
    noSlides: "0",
    maxSlides: "5",
    ...
    init: function(callback) {
      this._getAppConfig("slideIds").done(function(data) {
        if (data.ocs.data.data !== "") {
          OCA2.NMC_Welcome_Popup.Admin.slideIds = data.ocs.data.data;
          OCA2.NMC_Welcome_Popup.Admin.currentConfig = OCA2.NMC_Welcome_Popup.Admin.slideIds.split(",")[0];
        } else {
          OCA2.NMC_Welcome_Popup.Admin.noSlides = "1";
        }
        callback();
      });
    },
    ...
    addSlide: function(callback) {
      var slideIds = OCA2.NMC_Welcome_Popup.Admin.slideIds.split(",");
      if (slideIds.length < OCA2.NMC_Welcome_Popup.Admin.maxSlides) {
        var nextId = 1;
        ...
        OCP.AppConfig.setValue("nmc_welcome_popup", "slideIds", OCA2.NMC_Welcome_Popup.Admin.slideIds + "," + nextId, {
          success: function() {
            OCA2.NMC_Welcome_Popup.Admin.slideIds += "," + nextId;
            OCA2.NMC_Welcome_Popup.Admin.noSlides = "0";
            callback(nextId);
          }
        });
      }
    ...
    removeSlide: function(callback) {
      if (OCA2.NMC_Welcome_Popup.Admin.noSlides === "0") {
        var slideIds = OCA2.NMC_Welcome_Popup.Admin.slideIds.split(",");
        if (slideIds.length >= 1) {
          ...
          $.ajax({ url: OC.generateUrl("/apps/nmc_welcome_popup/ajax/slideSettings/" + this.currentConfig), type: "DELETE" }).done(function(data) {
            OCP.AppConfig.setValue("nmc_welcome_popup", "slideIds", slideIds.join(","), {
              success: function() {
                if (slideIds.length > 0) {
                  OCA2.NMC_Welcome_Popup.Admin.slideIds = slideIds.join(",");
                } else {
                  OCA2.NMC_Welcome_Popup.Admin.noSlides = "1";
                  OCA2.NMC_Welcome_Popup.Admin.slideIds = "1";
                }
                callback(config);
              }
            });
...
$(function() {
  ...
  $(".show-preview").click(function(event) {
    ...
    if (!image_name) {
      ...
      var cachebusterImg = Math.round((/* @__PURE__ */ new Date()).getTime() / 1e3);
        ...
        Object.keys(data).forEach(function(form_section) {
          if (form_section == en || form_section == de) {
            ...
            Object.keys(entries).forEach(function(configKey) {
              var element2 = $('#welcome_popup *[data-key="' + form_section + "_" + configKey + '"]');
              if (element2.is("input") && (element2.prop("type") === "text" || element2.prop("type") === "number" || element2.prop("type") === "url")) {
                element2.val(entries[configKey]);
              } else if (element2.is("textarea")) {
                element2.val(entries[configKey]);
              } else if (element2.prop("type") === "checkbox") {
